---
title: "Week 9 Worklog"
date: 2026-01-01
weight: 9
chapter: false
pre: " <b> 1.9. </b> "
---

### Week 9 Objectives:
* Attend advanced technical training modules aimed at shipping Multi-AZ High Availability apps.
* Understand global resolution mechanisms using Route53 and orchestrate resources programmatically through CLI scripts.

### Tasks to be carried out this week:
| Day | Task | Start Date | Completion Date | Reference Material |
| --- | --- | --- | --- | --- |
| 2 | - Performing offline work responsibilities and syncing project updates at the office. | 15/06/2026 | 15/06/2026 | |
| 3 | - Participating in the "Designing Highly Available Web Applications" technical training. | 16/06/2026 | 18/06/2026 | https://cloudjourney.awsstudygroup.com |
| 4 | - Studying Hybrid DNS resolution systems connecting private enterprise data networks with Amazon VPC using Route53. | 17/06/2026 | 19/06/2026 | |
| 5 | - Working with administration tools like AWS CLI within virtualized operating environments (Ubuntu/Windows). | 18/06/2026 | 20/06/2026 | |
| 6 | - Executing active DNS record routing setups and registry controls inside Route53. | 19/06/2026 | 20/06/2026 | |

### Week 9 Achievements:
* Acquired system design insights for deploying High Availability structures across Multi-Availability Zones with load balancers.
* Mastered critical DNS routing operations (record creation for A, CNAME, and MX properties) using AWS Route53.
* Structured operational shell automation files executing programmatic virtual server boots securely using terminal commands.